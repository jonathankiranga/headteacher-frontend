...
function LessonPlanModal({ plan, schoolId, onClose, onSaved }) {
  ...
  const [studentData, setStudentData] = useState({ students: [], classes: [] });
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (plan) {
        await updateLessonPlan(plan.plan_id, body);
      } else {
...
export default function LessonPlansPage() {
  ...
  async function handleDelete(id) {
    ...
    try { await deleteLessonPlan(id); loadPlans(); } catch (e) { /* ignore */ }
  }
      ...
      <div className="max-w-3xl mx-auto px-4 py-5">
        ...
        {loading ? (
          ...
          <div className="card p-12 text-center border border-gray-200">
            <p className="text-sm" style={{ color: '#888' }}>No lesson plans yet. Create your first one!</p>
          </div>
        ) : (
              ...
              <div key={plan.plan_id} className="card p-4">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    {plan.sub_strand_name && <p className="text-xs" style={{ color: '#888' }}>Sub-strand: {plan.sub_strand_name}</p>}
                  </div>
