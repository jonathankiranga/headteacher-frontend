...
function renderLevelBadge(level) {
  if (!level || !LEVEL_COLORS[level]) return <span style={{ color: '#999' }}>—</span>;
  const c = LEVEL_COLORS[level];
  return <span className="text-xs px-1.5 py-0.5 rounded font-medium" style={{ backgroundColor: c.bg, color: c.text }}>{level}</span>;
}
...
export default function StrandPerformancePage() {
  ...
  async function handleDownloadPdf() {
    ...
    try {
      ...
      // ========== SUMMARY STATS ==========
      // Collect dominant levels across sub-strands (CBC reports levels, never averages)
      const LEVEL_W = { EE: 4, ME: 3, AE: 2, BE: 1 };
      const topLevel = (levels) => {
        const counts = { EE: 0, ME: 0, AE: 0, BE: 0 };
        (levels || []).filter(Boolean).forEach(l => { if (counts[l] !== undefined) counts[l]++; });
        let best = null;
      ...
      areas.forEach(area => {
        (area.strands || []).forEach(strand => {
          (strand.sub_strands || []).forEach(sub => {
            ...
            if (sub.level) {
              assessedSubStrands++;
              allLevels.push(sub.level);
            }
          });
        });
      });
      const overallLevel = topLevel(allLevels) || '—';
      ...
      y += 5;
      doc.text(`Sub-strands Assessed: ${assessedSubStrands}/${totalSubStrands} | Overall Level: ${overallLevel}`, 14, y);
      y += 10;
      ...
      const colX = [
        ...
        colStart + colNameWidth + colStrandWidth + colSubWidth, // Level
        colStart + colNameWidth + colStrandWidth + colSubWidth + colLevelWidth // Students
      ];
      const tableEndX = colStart + colNameWidth + colStrandWidth + colSubWidth + colLevelWidth + colCountWidth;
      ...
      doc.text('Level', colX[3] + 1, y);
      doc.text('Students', colX[4] + 1, y);
      y += 4;
      doc.setDrawColor(0);
      ...
      for (const area of areas) {
        ...
        for (const strand of strands) {
          ...
          for (const sub of subStrands) {
            ...
            doc.text(sub.level || '—', colX[3] + 1, y);
            doc.text(String(sub.student_count || 0), colX[4] + 2, y);
            y += 4;
          }

          // Strand summary row
          const strandAssessed = subStrands.filter(s => s.level);
          if (strandAssessed.length > 0) {
            doc.setFont('helvetica', 'italic');
            doc.setFontSize(7);
            doc.text(`Strand Level: ${strand.level || '—'}`, colX[1] + 1, y);
            y += 4;
          }
        }

        // Area summary row
        const areaAssessed = strands.flatMap(s => s.sub_strands || []).filter(s => s.level);
        if (areaAssessed.length > 0) {
          doc.setFont('helvetica', 'bold');
          doc.setFontSize(7);
          doc.text(`Area Level: ${area.level || '—'}`, colX[0] + 1, y);
          y += 6;
        }
      ...
      y += 5;
      doc.text(`Overall Level: ${overallLevel}`, leftMargin, y);
      y += 5;
    ...
    <div className="max-w-6xl mx-auto px-4 py-6">
                      ...
                      <div key={strand.strand_id} className="pl-4 border-l-2" style={{ borderColor: '#E0E0E0' }}>
                        <div className="flex items-center justify-between py-2">
                          ...
                          <div className="flex items-center gap-2">
                            {renderLevelBadge(strand.level)}
                            <span className="text-xs" style={{ color: '#999' }}>{strand.counts?.total || 0} assessed</span>
                          </div>
                        </div>
      ...
      <HelpPanel open={showHelp} onClose={() => setShowHelp(false)} title="Strand Performance — Help">
        ...
        <HelpSection icon="📂" title="How to read it">
          ...
          strand, the individual sub-strands with their dominant performance
          level badge and the number of assessed students.
        </HelpSection>
