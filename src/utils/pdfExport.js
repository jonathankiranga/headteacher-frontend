...
// Draw a small colored level badge (filled rect + white/dark text)
...
export async function downloadAcademicPdf(report, childName, phone, term) {
  ...
  const colSubArea = 48;
  const cellH = 14; // row height: score line (7) + badge line (7)
  const headerH = 7;
  ...
  for (const area of learningAreas) {
    ...
    // Page break check: need at least header + one data row
    const neededH = headerH * 2 + cellH + 18; // area header + col header + 1 row + remarks
    if (y + neededH > pageH - M) { doc.addPage(); y = M; }

    // Area header bar (purple)
  ...
  // School stamp box (right, replaces date on HT side)
  doc.setDrawColor(160, 120, 200);
